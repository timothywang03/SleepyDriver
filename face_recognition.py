...
def main():
    ...
    while True:
        ...
        attention_queue.append(attention)

        if len(attention_queue) > 50:
            attention_queue.popleft()

        print(attention_queue.count(True), attention_queue.count(False))
        # outputs a boolean based on whether or not the car should be moving
        if len(attention_queue) >= 50 and attention_queue.count(True) < 35:
